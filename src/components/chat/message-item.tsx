...
import { User, Bot, Brain, FileAudio, Trash2, Users, Settings } from "lucide-react"; 
...
export default function MessageItem({ 
  ...
  let avatarInitial = message.sender === 'user' ? 'U' : 'AI';
  let avatarSrc = `https://placehold.co/40x40.png?text=${avatarInitial}`;
  let avatarAlt = message.sender === 'user' ? "User" : "MetalAI";
  let aiHint = "profile person";

  if (isUser) {
    avatarInitial = "U"; // Or use display name initial
    ...
    avatarInitial = "O"; // Or fetch other user's display name initial
    avatarSrc = `https://placehold.co/40x40.png?text=O`;
    avatarAlt = "Other User";
    aiHint = "profile person";
  }

  if (message.deletedForMe) {
    return null; // Don't render if deleted for me (client-side hide)
  }

  const handleDeleteForMe = () => {
    if(conversationId) onDeleteMessageForMe(conversationId, message.id);
    setPopoverOpen(false);
  };

  const handleDeleteForEveryone = () => {
     if(conversationId) onDeleteMessageForEveryone(conversationId, message.id);
    setPopoverOpen(false);
  };
      ...
      case "audio":
        return (
          <div className="flex flex-col items-start">
            ...
            {message.sender === 'user' && !message.fileUrl?.startsWith('https://firebasestorage.googleapis.com') && <p className="text-xs text-muted-foreground/70 mt-1">(Local preview. Upload needed for others.)</p>}
          </div>
        );
      case "video":
        return (
  ...
  return (
    ...
    >
      ...
      <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
        <PopoverTrigger asChild disabled={message.isDeleted || message.isLoading}>
          <div
            className={cn(
              "max-w-[70%] rounded-lg px-4 py-2 shadow-md cursor-pointer",
              isUser
                ? "bg-primary text-primary-foreground rounded-br-none"
                : isMetalAI 
                ? "bg-secondary text-secondary-foreground rounded-bl-none"
                : "bg-card text-card-foreground rounded-bl-none",
              (message.isDeleted || message.isLoading) && "cursor-default"
            )}
          >
            {messageContent()}
        ...
        {!message.isDeleted && !message.isLoading && (
            <PopoverContent className="w-auto p-1">
                ...
                {isUser && ( // Only sender can delete for everyone
                <Button variant="ghost" size="sm" className="w-full justify-start" onClick={handleDeleteForEveryone}>
